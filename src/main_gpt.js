...
// --- Summarize FHIR Data for Chat Context ---
...
async function sendChatMessage(msg) {
  ...
  // Add FHIR context as system prompt
  const fhirContext = [
  ...
  try {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        model: "gpt-3.5-turbo",
        messages: messages.slice(-10), // system + last 9 messages
      }),
    });
